...
const LeaderDashboard = () => {
  const{displayName} = React.useContext(ProfileContext);
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await fetch(`${config.azureApiUrl}/api/leaders-page/clients`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      if (selectedClientId) {
        try {
          const response = await fetch(`${config.azureApiUrl}/api/leaders-page/client/${selectedClientId}/projects`);
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
          }
          const data = await response.json();
