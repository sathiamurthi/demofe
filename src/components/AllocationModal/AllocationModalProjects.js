...
const AllocationModalProjects = ({
  ...
  useEffect(() => {
    const fetchEmployeesInitial = async () => {
      try {
        const response = await axios.get(`${config.azureApiUrl}/api/employees/search?query=`);
  ...
  useEffect(() => {
    const fetchTotalAllocation = async () => {
      ...
      try {
        const response = await axios.get(`${config.azureApiUrl}/api/employeeAllocations/${formData.employeeId}`, {
          params: {
  ...
  useEffect(() => {
    const fetchModalData = async () => {
      ...
      setFetchError(null);
      try {
        const response = await axios.get(`${config.azureApiUrl}/api/modal/data`);
  ...
  const fetchEmployeeDetails = async (employeeId) => {
    try {
      const response = await axios.get(`${config.azureApiUrl}/api/employees/${employeeId}`);
      if (response.data) {
        setFormData(prev => ({
          ...prev,
    ...
    const fetchEmployeeDetails = async () => {
      try {
        ...
        if (formData.employeeId && !formData.employeeName) {
          response = await axios.get(`${config.azureApiUrl}/api/employees/${formData.employeeId}`);
          setFormData(prev => ({
            ...prev,
            employeeName: response.data.EmployeeName || '',
          }));
        } else if (formData.employeeName && !formData.employeeId) {
          response = await axios.get(`${config.azureApiUrl}/api/employees/search?query=${formData.employeeName}`);
          setFormData(prev => ({
            ...prev,
  ...
  const fetchEmployees = useCallback(_.debounce(async (query, type) => {
    ...
    try {
      const response = await axios.get(`${config.azureApiUrl}/api/employees/search`, {
        params: { query }
      });
  ...
  const isFormValid = () => {
    ...
    // Conditional validation for Billing Rate
    if (billedCheck === 'Yes') {
      if (
        billingRate === '' ||
        isNaN(billingRate) ||
        parseFloat(billingRate) <= 0
      ) {
        return false;
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const overlapResponse = await axios.get(`${config.azureApiUrl}/api/employeeAllocationDetails/${formData.employeeId}/allocations`);
      ...
      if (allocationData) {
        ...
        const totalAllocationResponse = await axios.get(`${config.azureApiUrl}/api/employeeAllocations/${formData.employeeId}`, {
          params: {
            startDate: formData.startDate,
            endDate: formData.endDate
          }
      ...
      if (allocationData && allocationData.AllocationID) {
        // Editing existing allocation
        await axios.put(`${config.azureApiUrl}/api/allocations/${allocationData.AllocationID}`, payload);
        toast.success('Allocation updated successfully!');
      } else {
        // Adding new allocation
        await axios.post(`${config.azureApiUrl}/api/api/allocate`, payload);
