  ...
  useEffect(() => {
    const fetchEmployeeDetails = async () => {
      if (formData.employeeId && !employeeData) {
        try {
          const response = await axios.get(`${config.azureApiUrl}/api/employees/${formData.employeeId}`);
          if (response.data) {
            setFormData(prev => ({
              ...prev,
    ...
    const fetchEmployeeId = async () => {
      if (formData.employeeName && !employeeData) {
        try {
          const response = await axios.get(`${config.azureApiUrl}/api/employees?name=${formData.employeeName}`);
