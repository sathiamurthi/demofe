// src/components/ExcelPreviewModal/ClientsTableUpload.js
import React, { useState, useEffect } from 'react';
import { Modal, Table, Button, Icon, Message, Progress, List } from 'semantic-ui-react';
import PropTypes from 'prop-types';
import config from "../../Config.json";

const ClientsTableUpload = ({ open, onClose, data }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${config.azureApiUrl}/api/clients-excel/upload`, {
        method: 'POST',
        headers: {
  ...
  return (
    <Modal open={open} onClose={onClose} size="fullscreen">
      <Modal.Header>Uploaded Clients Data</Modal.Header>
      <Modal.Content scrolling>
        ...
        {(submissionStatus === 'success' || submissionStatus === 'partial') && (
          <Message positive>
            ...
            <p>{successCount} out of {data.length} client records have been successfully submitted.</p>
            {successNames.length > 0 && (
              <div>
                <strong>Successfully Submitted ClientNames:</strong>
                <List bulleted>
          ...
          <Message warning>
            ...
            <List>
              {failureDetails.map((fail, idx) => (
                <List.Item key={idx}>
                  <strong>ClientName:</strong> {fail.ClientName} - <strong>Error:</strong> {fail.message}
                </List.Item>
              ))}
            </List>
          </Message>
...
ClientsTableUpload.propTypes = {
  open: PropTypes.bool.isRequired,
  onClose: PropTypes.func.isRequired,
  data: PropTypes.arrayOf(PropTypes.object).isRequired,
};

export default ClientsTableUpload;
