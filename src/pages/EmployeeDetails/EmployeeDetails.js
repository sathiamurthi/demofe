...
const EmployeeDetails = () => {  // Accept userRole as a prop
  ...
  const fetchEmployeeData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${config.azureApiUrl}/api/employeeDetails/${id}`);
  ...
  const fetchAllocations = async (currentFilter) => {
    try {
      setLoading(true);
      const response = await axios.get(`${config.azureApiUrl}/api/employeeAllocationDetails/${id}/allocations`, {
  ...
  const handleDeleteAllocation = async (allocationId) => {
    try {
      await axios.delete(`${config.azureApiUrl}/api/allocations/${allocationId}`);
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      const fetchPromises = filtersList.map(filterType =>
        axios.get(`${config.azureApiUrl}/api/employeeAllocationDetails/${id}/allocations`, {
      ...
      <div className='right-content'>
        <div className='breadcrumb'>
          ...
          <h2 
            ...
            onClick={() => navigate('/allocations/employees')}
            style={{ cursor: 'pointer' }}
          >
            Employees
          </h2>
